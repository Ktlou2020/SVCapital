...
async function runMaturityProcessing() {
  ...
  // Pool status is updated inside each investment's transaction (FIX 10).
  ...
  try {
    ...
    for (const inv of investments) {
      try {
        ...
        // Mark the pool as matured inside this transaction (FIX 10).
        await client.query(
        ...
        // All money movement done — close out the maturity in the SAME transaction (FIX 6).
        await client.query(
        ...
        // Notify the investor (outside transaction — non-blocking).
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
async function creditWallet(client, inv, amount, returnPortion, description, ref) {
  const amt = round2(amount);
  if (amt <= 0) return;
  const reference = ref || 'MAT-' + inv.id;
  await client.query(
  ...
  await client.query(
    ...
    [inv.investor_id, amt, reference, description, inv.id]
  );
}
...
async function reinvestAmount(client, inv, amount, productType, sourcePoolName) {
  ...
  if (amt <= 0) return;
  try {
    // Step 1: Find candidate pool (capacity check deferred until lock acquired).
    ...
    if (!candidates.length) {
      // No open pool for this product — pay the amount to the wallet (FIX 8).
      await creditWallet(client, inv, amt, 0, `Maturity payout — ${sourcePoolName} (no open ${productType || ''} pool to reinvest into)`, 'MAT-FALLBACK-' + inv.id);
      console.log(`[maturity] no open ${productType} pool — paid R${amt} to wallet for ${inv.investor_id}`);
      return;
    }

    // Step 2: Lock pool row and re-check capacity after acquiring lock (FIX 9).
    ...
    if (lockedPool.max_investment !== null &&
        (Number(lockedPool.current_invested || 0) + amt) > Number(lockedPool.max_investment)) {
      // Pool at capacity — fall back to wallet (FIX 8).
      await creditWallet(client, inv, amt, 0, `Maturity payout — ${sourcePoolName} (no open ${productType || ''} pool to reinvest into)`, 'MAT-FALLBACK-' + inv.id);
      console.log(`[maturity] no open ${productType} pool — paid R${amt} to wallet for ${inv.investor_id}`);
      return;
    }
    ...
    throw err; // FIX 7: propagate so maturity_processed_at is NOT set
  }
}
